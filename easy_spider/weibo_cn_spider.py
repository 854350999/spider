...
import pymysql
db = pymysql.connect(host='localhost', user='root', password='root', db='mysql', charset='utf8mb4')
cursor = db.cursor()
sql_insert = 'INSERT INTO weibo' \
             '(id,page,weiBoCount,guanZhuCount, fenSiCount, content,' \
             ' dianZanCount, zhuanFaCount, piLunCount, shijian, ly, shifo) ' \
...
##打开Firefox浏览器
    ...
    time.sleep(2)
    username.clear()
    username.send_keys('13365948372')#输入自己的账号
    #找到输入密码的地方，然后送入你的密码
    password = browser.find_element_by_css_selector('#loginPassword')
    time.sleep(2)
    password.send_keys('xiaoqingge939528')
    #点击登录
    browser.find_element_by_css_selector('#loginAction').click()
    ##这里给个15秒非常重要，因为在点击登录之后，新浪微博会有个九宫格验证码，下图有，通过程序执行的话会有
# 点麻烦（可以参考崔庆才的Python书里面有解决方法），这里就手动
    time.sleep(15)
except:
    print('########出现Error########')
finally:
    ...
    for divs in divss:
        ...
        # 这里为转发，其他和上面一样
            ...
            if '来自' in span:
                ...
                laiYuan = []
        print(laiYuan)
        #cursor.execute(sql_insert, (uid, pageSize, weiBoCount, guanZhuCount, fenSiCount, content,
                                    #dianZan, zhuanFa, pinLun, faBuTime, span, yuanChuang))
        #db.commit()
    time.sleep(2)
    print(i)
